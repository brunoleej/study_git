# cnn mnist
# 여러개 gpu를 돌린다. > vscode 두 개를 동시에 돌릴 수 있다.
...
if gpus :
    try :
        ...
        # gpu가 두 개 이상일 때 : 다른 파일에서 프로그램 돌리고 싶다면 gpus[*] gpu 숫자만 바꾸면 된다.
...
#2. Modeling
# regularizer = tf.compat.layers.l2_regularizer(scale=0.01)

# layer 1
w1 = tf.compat.v1.get_variable("w1", shape=[3, 3, 1, 128],initializer=tf.compat.v1.initializers.he_normal())
                #initializer=tf.keras.initializers.he_uniform())#, regularizer=regularizer) # 4차원 맞춘다. : 3,3 kernel_size / 1 channel / 32 filter
...
# layer 2
w2 = tf.compat.v1.get_variable("w2", shape=[3, 3, 128, 64],initializer=tf.compat.v1.initializers.he_normal())
                    #initializer=tf.keras.initializers.he_uniform(), regularizer=regularizer) # 4차원 맞춘다. : 3,3 kernel_size / 32 위의 output이 현재 레이어의 input  / 32 filter
...
# layer 3
w3 = tf.compat.v1.get_variable("w3", shape=[3, 3, 64, 32],initializer=tf.compat.v1.initializers.he_normal())
#                initializer=tf.keras.initializers.he_uniform(), regularizer=regularizer) 
...
# layer 4
w4 = tf.compat.v1.get_variable("w4", shape=[3, 3, 32, 32],initializer=tf.compat.v1.initializers.he_normal())
#                    initializer=tf.keras.initializers.he_uniform(), regularizer=regularizer) 
...
# layer 5
w5 = tf.compat.v1.get_variable("w5", shape=[2*2*32,64],initializer=tf.compat.v1.initializers.he_normal())
#                    initializer=tf.contrib.keras.initializers.he_normal(), regularizer=regularizer)
...
optimizer = tf.compat.v1.train.AdamOptimizer(1.7e-5).minimize(loss)
